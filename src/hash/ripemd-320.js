 ...
 */
const ripemd320MainLoop = (state, buffer) => {
  // RIPEMD-320 includes a parallel processing rounds
  // which denotes as left rounds and right rounds
  let [al, bl, cl, dl, el, ar, br, cr, dr, er] = state;

  for (let i = 0; i < 80; i += 1) {
    // RIPEMD-320 follows big little mode
    const gl = zl[i];
    const mgl =
      (buffer[gl * 4 + 3] << 24) |
      (buffer[gl * 4 + 2] << 16) |
...
export class RIPEMD320Hash extends MD4HashBase {
  static title = 'ripemd-320';

  constructor() {
    super(RIPEMD320Hash, 'RIPEMD-320');
  }

  mainLoop() {
    ripemd320MainLoop(this.state, this.buffer);
  }

  initState() {
    this.state = new Uint32Array(10);
  }
  ...
  static hash(data) {
    return new RIPEMD320Hash().hash(data);
  }
}

export default RIPEMD320Hash;
